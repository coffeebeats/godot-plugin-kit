##
## Screens is a shared library for reaching the `StdScreenManager` which mounted a scene.
## A screen's scene sits beneath its manager, so a menu finds the manager by walking its
## ancestors, as `StdScreenPusher` does, rather than through a game-owned global.
##
