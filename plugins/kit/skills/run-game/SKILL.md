...
- **A sandboxed harness cannot write `user://`.** Settings and saves the game writes are dropped, and the engine's log rotation fails at startup. Reads are unaffected, which is what makes it plausible: existing saves load, and only the write is missing, so a run that looks clean persists nothing. Grant the game's user directory — `OS.get_user_data_dir()`, which the `use_custom_user_dir` project setting moves — to the harness to persist anything. Under Codex that is `--add-dir <dir>`, or `sandbox_workspace_write.writable_roots` in `.codex/config.toml`. Only GDScript's `SCRIPT ERROR` aborts a wait, so the engine errors a sandbox provokes appear in `logs` without failing the run.
