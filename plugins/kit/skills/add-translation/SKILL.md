...
`locale/translations` in `project.godot` lists every `.mo` the game loads. The game's catalogue is the directory holding the **first entry that is not under `addons/`** — `project/locale/` in a repository created from the template.
